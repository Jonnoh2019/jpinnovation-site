(() => {
  ...
  const VERSION = "access-tier-fix-20260722";
  ...
  function getUser() {
    try { return normalizeUser(typeof currentUser === "function" ? currentUser() : null); } catch { return null; }
  }

  function hasHub(user = getUser()) {
    const role = strictRole(user);
  ...
  function safeView(view, user = getUser()) {
    if (isAllowedView(view, user)) return view;
    return "dashboard";
  }

  function toast(title, detail = "", isError = false) {
    if (typeof showSuccessToast === "function" && !isError) {
      showSuccessToast(title, detail);
      return;
    }
    const old = document.querySelector(".jp-access-tier-toast");
    if (old) old.remove();
  ...
  function applyAccessUi() {
    ...
    document.documentElement.classList.toggle("hub-member-session", hasHub(user));
    ...
    document.querySelectorAll(".nav-link").forEach((button) => {
      ...
      button.classList.toggle("hidden", !isAllowedView(view, user) || (client && !CLIENT_VIEWS.has(view)));
    });
  ...
  function profileToUser(row) {
    ...
    const user = {
      ...
      role: row.account_type || "client",
      account_type: row.account_type || "client",
      membershipStatus: row.membership_status || "free",
      membership_status: row.membership_status || "free",
      vetted: Boolean(row.vetted_at),
      vetted_at: row.vetted_at || "",
      verified: Boolean(row.vetted_at),
  ...
  function findAccount(button) {
    ...
    let row = null;
    try { row = (secureAdminProfiles || []).find((profile) => profile.user_id === userId || clean(profile.email) === email); } catch {}
    if (row) return profileToUser(row);
    try {
      const local = (state.users || []).find((user) => user.id === userId || user.user_id === userId || clean(user.email) === email);
  ...
  async function manageAccount(member, action, reason = "") {
    ...
    else if (action === "verify") Object.assign(params, { p_account_type: strictRole(member) === "client" ? "client" : null, p_membership_status: strictRole(member) === "client" ? "free" : "active" });
    ...
    let result = null;
    try {
      result = await rpc("admin_manage_account", { p_user_id: userId, p_action: action, p_reason: reason || "" });
    } catch (error) {
  ...
  function shouldOwnAdminAction(button) {
    const action = button?.dataset?.adminAction;
    return ["upgrade", "downgrade", "suspend", "restore", "reactivate", "remove"].includes(action || "");
  }
  ...
  async function handleAdminButton(button) {
    ...
    let reason = action === "remove" ? "Archived by JP Innovation admin" : (action === "suspend" ? "Suspended by JP Innovation admin" : "");
    ...
    try {
      ...
      const messages = {
        upgrade: [name + " upgraded.", "Hub access is now active in the database."],
        downgrade: [name + " moved to Client Portal.", "Hub access has been removed."],
        suspend: [name + " suspended.", "Access is blocked until reactivated."],
        restore: [name + " reactivated.", "Access has been restored."],
        reactivate: [name + " reactivated.", "Access has been restored."],
        remove: [name + " archived.", "The account has been removed from active management."]
      };
  ...
  function installWrappers() {
    if (typeof hasActiveHubAccess === "function" && !hasActiveHubAccess.jpAccessTierWrapped) {
      hasActiveHubAccess = function jpHasActiveHubAccess(user = getUser()) { return hasHub(user); };
      hasActiveHubAccess.jpAccessTierWrapped = true;
    }
    if (typeof isClientPortalContext === "function" && !isClientPortalContext.jpAccessTierWrapped) {
      isClientPortalContext = function jpIsClientPortalContext(user = getUser()) { return inClientContext(user); };
      isClientPortalContext.jpAccessTierWrapped = true;
    }
    if (typeof currentUser === "function" && !currentUser.jpAccessTierWrapped) {
      const baseCurrentUser = currentUser;
      currentUser = function jpCurrentUser() { return normalizeUser(baseCurrentUser()); };
      currentUser.jpAccessTierWrapped = true;
    }
    if (typeof syncSecureSession === "function" && !syncSecureSession.jpAccessTierWrapped) {
      const baseSync = syncSecureSession;
      syncSecureSession = async function jpSyncSecureSession() {
        const user = normalizeUser(await baseSync());
        applyAccessUi();
        return user;
      };
      syncSecureSession.jpAccessTierWrapped = true;
    }
    if (typeof renderView === "function" && !renderView.jpAccessTierWrapped) {
      const baseRender = renderView;
      renderView = function jpRenderView(view) {
        const user = getUser();
        const next = safeView(view || "dashboard", user);
        if (next !== view) {
          toast("Access level updated.", "That area is not available for this account type.", true);
        }
        cleanOverlays();
        return baseRender(next);
      };
      renderView.jpAccessTierWrapped = true;
    }
    if (typeof setLoggedInView === "function" && !setLoggedInView.jpAccessTierWrapped) {
      const baseLoggedIn = setLoggedInView;
      setLoggedInView = function jpSetLoggedInView() {
        normalizeUser(getUser());
        const result = baseLoggedIn();
        applyAccessUi();
        return result;
      };
      setLoggedInView.jpAccessTierWrapped = true;
    }
  }
  ...
  function installAdminActionCapture() {
    ...
    window.addEventListener("click", (event) => {
      ...
      handleAdminButton(button).catch((error) => {
        console.error(`[${VERSION}] account action failed`, error);
        toast("Account update failed.", error.message || "Please try again.", true);
        try { if (typeof renderView === "function") renderView("admin"); } catch {}
      });
    }, true);
  }
  ...
  function delayedAuthPrompt() {
    ...
    window.setTimeout(() => {
      ...
      if (anyOpen) return;
      if (typeof openAuth === "function") openAuth(params.get("register") === "1" ? "register" : "signin");
    }, 2000);
  }
  ...
  function install() {
    addStyles();
    installWrappers();
    installAdminActionCapture();
    applyAccessUi();
    delayedAuthPrompt();
    const observer = new MutationObserver(applyAccessUi);
    observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["class", "data-view"] });
    window.addEventListener("focus", applyAccessUi);
    window.addEventListener("pageshow", applyAccessUi);
  }
